...
class SignatureJobProcessor @Inject constructor(
    ...
    override val supportedType: JobType = JobType.SIGNATURE
    ...
    override suspend fun process(jobId: UUID): ProcessorResult = processingGate.run {
        withContext(Dispatchers.IO) {
            ...
            if (plan.jobId != jobId || plan.signatureOptions == null) {
                return@withContext ProcessorResult.Failure(PhotoJobProcessor.ERROR_PLAN_INVALID)
            }
            val extension = when (plan.output.format) {
                ...
                OutputFormat.PDF ->
                    return@withContext ProcessorResult.Failure(
                        PhotoJobProcessor.ERROR_PLAN_INVALID,
                    )
            }
            ...
            final.delete()
            try {
                val prepared = signatureProcessor.prepare(input, partial, plan)
                ...
                outputs.add(
                    OutputArtifact(
                        ...
                        displayName = "FormReady-signature-$jobId.$extension",
